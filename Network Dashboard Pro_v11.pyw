...
class NetworkDashboard:
    ...
    def manage_tools_category(self, cat):
        win = tk.Toplevel(self.root); win.title(f"Outils : {cat}");
    ...
    def manage_tools_device(self, cat, idx):
        ...
        win.title("Outils Perso"); win.geometry("200x200")
    ...
    def set_extra(self, cat, idx, key, hint):
        d = self.data[cat]["devices"][idx]; old = d.get(key, "")
        if not old:
        ...
        if val is not None: 
            d[key] = val; self.save_all(); self.refresh_ui()
    ...
    def add_manual(self):
        c = self.cat_var.get()
        if c in self.data: self.data[c]["devices"].append({"name": self.e_name.get(), "ip": self.e_ip.get(), "mac": self.e_mac.get()}); self.save_all(); self.refresh_ui()
        
    def delete_dev(self, cat, idx): 
        if messagebox.askyesno("Suppr", "Supprimer ?"): del self.data[cat]["devices"][idx]; self.save_all(); self.refresh_ui()
    ...
    def add_category(self):
        ...
        if n:
            ...
            self.update_cat_menu(); self.save_all(); self.refresh_ui()
        
    def delete_cat(self, cat):
        if messagebox.askyesno("X", f"Supprimer {cat} ?"): del self.data[cat]; self.save_all(); self.update_cat_menu(); self.refresh_ui()
        
    def edit_name(self, cat, idx):
        n = simpledialog.askstring("Nom", "Nom :", initialvalue=self.data[cat]["devices"][idx]["name"])
        if n: self.data[cat]["devices"][idx]["name"] = n; self.save_all(); self.refresh_ui()
